...
4. （可选，纯 UX）`index.html` 里还有一份前端的邮箱提示：
   ```js
   const ALLOWED_EMAILS = ["你的邮箱@example.com"];
   ```
   同样填你的邮箱。这段只是让别人在你网页上输错邮箱时能立刻看到"不在允许名单里"的提示，体验更友好而已；真正兜底防滥用的是上一步的 `$users` 规则，就算没这一步前端提示，第 3 步的后台规则也已经够用了。
